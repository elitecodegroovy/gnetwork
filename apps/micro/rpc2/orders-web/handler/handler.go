...
import (
	...
	invS "github.com/elitecodegroovy/gnetwork/apps/micro/rpc2/inventory-srv/proto/inventory"
	orders "github.com/elitecodegroovy/gnetwork/apps/micro/rpc2/orders-srv/proto/order"
...
func New(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method != "POST" {
		log.Logf("非法请求")
		http.Error(w, "非法请求, userId doesn't exist!", 400)
		return
	}
	...
	log.Logf("userId :%+v", session.GetSession(w, r).Values["userId"])
	// 调用后台服务
	rsp, err := serviceClient.New(context.Background(), &orders.OrderRequest{
		BookId: bookId,
