...
import (
	...
	ordS "github.com/elitecodegroovy/gnetwork/apps/micro/rpc2/orders-srv/proto/order"
...
func (s *service) PayOrder(orderId int64) (err error) {
	// 获取支付单
	orderRsp, err := ordSClient.GetOrder(context.TODO(), &ordS.OrderRequest{
		OrderId: orderId,
	})
	if err != nil {
	...
	// 确认出库
	invRsp, err := invClient.Confirm(context.TODO(), &invS.InventoryRequest{
